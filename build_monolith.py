#!/usr/bin/env python3
# build_monolith.py — собирает финальный app.js из всех частей
import os, sys

BASE = os.path.dirname(__file__)
APP_OUT  = os.path.join(BASE, 'app.js')
SYNC_OUT = os.path.join(BASE, 'api', 'sync.js')

# ─────────────────────────────────────────────────────────────────────────────
# APP.JS — полный монолит
# ─────────────────────────────────────────────────────────────────────────────
APP_CODE = r'''/* ══════════════════════════════════════════════════════════════
...
# ─────────────────────────────────────────────────────────────────────────────
# API/SYNC.JS — обновлённая версия с createClient
# ─────────────────────────────────────────────────────────────────────────────
SYNC_CODE = r'''/**
...
# ─────────────────────────────────────────────────────────────────────────────
import os
app_size  = os.path.getsize(APP_OUT)
sync_size = os.path.getsize(SYNC_OUT)
print(f'\n✅ app.js  → {app_size:,} bytes ({app_size/1024:.1f} KB)')
print(f'✅ sync.js → {sync_size:,} bytes ({sync_size/1024:.1f} KB)')

with open(APP_OUT, 'r', encoding='utf-8') as f:
    code = f.read()

CHECKS = [
    ('StateManager',      'StateManager Pub/Sub'),
    ...
    ('NLP.parse',         'NLP Parser'),
    ('loadDemoData',      'Demo Data'),
    ('DOMContentLoaded',  'Init block'),
    ('island-btn',        'Island nav (not drawer)'),
    ('lifeos-goals',      'localStorage key v4'),
    ('ring-fill',         'Timer ring ID'),
    ('toast-stack',       'Toast stack'),
    ('createClient',      'KV createClient'),
]

BAD_IDS = ['popup-stack','search-bar-wrap','ring-fill-el','timer-time-display','drawer-link','view-panel']
...
print('\n── Old IDs (должны отсутствовать) ──')
for old in BAD_IDS:
    found = old in code
    print(f"  {'⚠️  FOUND' if found else '✅ absent'}: '{old}'")
